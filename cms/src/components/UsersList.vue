<template>
    <table>
        ...
        <tr v-for="user in users" class="dataRow">
            ...
            <td><strong>{{ user.auth ? user.auth : "N/A" }}</strong></td>
            <td class="editCell"><button @click="$emit('editUser', user.id)" class="editCellBtn" >Edit</button></td>
        </tr>
...
const update = () => {
    ...
    Users.getAllUsers().then((data) => {users.value = data});
}

const userData = Users.getAllUsers();
userData.then((data) => { users.value = data });
